...
{   
    ...
    ofstream myfile1;
    ofstream myfile2;
    myfile1.open ("sur_fixA_A_BDon_10k.dump");
...
//         if (rand_BD_GC>1/1000.0)
        if (rand_BD_GC>1/1000.0 && stop == 1)
        {
            ...
            for (i=0; i<Np; i++)
            {
                ...
                if (i==0){X1_new=X_new;Y1_new=Y_new;Z1_new=Z_new;}
                if (i==1){X2_new=X_new;Y2_new=Y_new;Z2_new=Z_new;}

                double dist = sqrt(SQ(X2_new-X1_new)+SQ(Y2_new-Y1_new)+SQ(Z2_new-Z1_new));

                if (dist <= 12.5 && i==1 && Z2_new>7.1)
                {
                    P[i].x=X_new;
                    P[i].y=Y_new;
                    P[i].z=Z_new;
