...
struct Particles
{
    ...
    //double bfb_a1f; //for three strand  comples; first a1 a2 bridge within the colloids, then bridge with the surface
    //double bfb_a2f; //for three strand  comples; first a2 a1 bridge within the colloids, then bridge with the surface
    Pairs Pn[maxNp];
};
Particles P[maxNp];
...
int main()
{
    ...
    //int NsC= Density_linkers*Lx*Ly; //2951; //1432; // To have same receptor density as linker density;

    double V=Lx*Ly*Lz;
    double DG0_surface_star=beta*DG0_surface-log(1/rho0*CUB(L));
    double DG0_star=beta*DG0-log(1/rho0*CUB(L));
    //double DG0_star=-10.0;
    double DG0_L_star=beta*DG0_L-log(1/rho0*CUB(L));

    //cout << Np << " " <<N << " " << Rc << " " << L << " " << R2s << " " << Lx << " " << Ly << " " << Lz << " " << beta << " " << Delta_tB << " " << t_tot << " " << DG0_surface << " " << DG0 << " " << DG0_L << " " <<  rho0 << " " << rho_ideal << " " << Na1 << " " << Na2 << endl;
    //exit(0);
    int nfS_C;
    ...
    ofstream myfile1;
    ofstream myfile2;
    //myfile.open ("test.dump");
    myfile1.open ("k0k1_blue_clean_7.0.dump");
    //myfile2.open ("surface_one_overlap_one_free-10_2ndp.dump");
    //myfile2.open ("A_2S_bridge_surface.txt");
...
//     printf("FreeEnergy_bulk=%lf\n",FreeEnergy_bulk);
    //cout << "Gas_phase= " <<  FreeEnergy_bulk << endl;
    //exit(0);
...
/**************************************  Brownian dynamics  ****************************/
        ...
        if (rand_BD_GC<1/1000.0)
        {
            ...
            for (i=0; i<Np; i++)
            {
                ...
                //{{\dot {X}}(t)=-\nabla U(X)D/k_{B}T+{\sqrt {2D}}R(t).} ; D=KbT/gamma;
                //if (X_new>Lx*0.5) X_new=X_new-Lx;

                //x = x + Force(x,dwPar)*beta*Ddt + sqrtDdt*randn;
                //  Ddt = 0.001;        % Diffusion coefficient times the time step
                    //sqrtDdt = sqrt(Ddt);
                ...
                //if      (Z_new <  0)    Z_new = Z_new + Lz;
                //else if (Z_new >  Lz)   Z_new = Z_new - Lz;
...
/**************************************  Particle insertion or delete  ****************************/
        else if (rand_BD_GC<1/1000.0)
        {
            ...
            if (rand_insertion_delete <0.5) //insertion
            {
                ...
                //cout << "Count_depo= " << count_deposition_ne <<endl;
                if (count_deposition_ne==Np)
                {
                    ...
                    //for (i=0; i<Np; i++) myfile << P[i].z << " " << P[i].nf_a1_old << " " << P_cpy[i].nf_a1_old << " " << P[i].nf_a2_old << " " << P_cpy[i].nf_a2_old << " "  << P[i].total_bridge_WC << " " << P_cpy[i].total_bridge_WC << " " << P[i].Pn[i].nl_d << " " <<  P_cpy[i].Pn[i].nl_d << endl;
            ...
            else if (rand_insertion_delete >=0.5)
            {
                ...
                //cout << " Rand_part= " << Rand_particle << endl;
                ...
                //cout << "count_deposition_ne= " << count_deposition_ne << endl;
                ...
                if (count_deposition_ne==Np-1)
                {
                    ...
                    //cout << " P[i_new].x= " << P[Np-1].x << " " << P[Rand_particle].x << " " << Np_new <<endl;
...
/**************************************  Data saved  ****************************/
        ...
        myfile1 << "0 " << Ly << " 0" << endl;
        myfile1 << "0 " << Lz << " 0" << endl;
        //myfile1 << "ITEM: ATOMS type id x y z Fx Fy Fz nbS nl a1_in_br a2_in_br nb nn nfa1 nfa2 nfb1 nfb2 lfbs  omega_i" <<  endl;
        ...
        //myfile2 << "r eij wij" <<  endl;
        //myfile2 << t <<  endl;
        //myfile2 << "ITEM: NUMBER OF ATOMS" <<  endl;
        //myfile2 << Np <<  endl;
        //myfile2 << "ITEM: BOX BOUNDS xy xz yz" <<  endl;
        //myfile2 << "0 " << Lx << " 0" << endl;
        //myfile2 << "0 " << Ly << " 0" << endl;
        //myfile2 << "0 " << Lz << " 0" << endl;
        //myfile2 << "nbS nl nb nn nfA nfB lfb" <<  endl;
        ...
        for(i=0; i<Np; i++)
        {
            ...
            for (j=0; j<P[i].maxnn; j++)
            {
                ...
                {
                    //tot_bridges=tot_bridges+P[i].Pn[index].nb_d;
           ...
           //myfile1 << "1" << " " << i+1 << " " << P[i].x << " " << P[i].y << " " << P[i].z << " " << P[i].Fx << " " << P[i].Fy << " " << P[i].Fz << " " << P[i].nbS_d << " " << P[i].Pn[i].nl_d << " " << tot_bridges <<endl;


            //myfile1 << P[i].type << " " << i+1 << " " << P[i].x << " " << P[i].y << " " << P[i].z << " " << P[i].Fx << " " << P[i].Fy << " " << P[i].Fz << " " << P[i].nbS_d << " " << P[i].Pn[i].nl_d << " " << tot_a1_in_Bridge << " " << tot_a2_in_Bridge << " " << tot_bridges << " " << P[i].maxnn << " " <<P[i].nf_a1_old << " " << P[i].nf_a2_old << " " <<P[i].nf_b1_old << " " << P[i].nf_b2_old << " " << P[i].lfb  << " " << P[i].omega_i  << endl;
...
//             printf("dist: A_2s_bridge_sur:  %lf %lf\n",P[1].z-P[0].z,P[0].nbS_d);

            //myfile2 << P[i].nbS_d << " " << P[i].Pn[i].nl_d << " " << tot_bridges << " " << P[i].maxnn << " " <<P[i].nf_a1_old << " " << P[i].nf_a2_old << " " << P[i].lfb << " " << P[i].bfb << endl;
            //myfile2 << " " << P[i].Pn[index].d << " " << eij << " " << wij << endl;
    ...
    for (i=0; i<Np; i++)
    {
        ...
        for (j=0; j<P[i].maxnn; j++)
        {
            ...
            else if (r<(2*Rc+1.9*L) && r>=2*Rc+L) ////
            {
                ...
                //overlap(2*Rc+L,D1,D1_,&ov);////modified
                //overlap_prime(2*Rc+L,D1,D1_,&ov_prime);////modified
...
//         printf("P[%d].omega_is=%lf P[index_cs].e_is=%lf\n",index_cs,P[index_cs].omega_is,P[index_cs].e_is);
...
{
//     double DG0t=-14.0;
//     double DG0_2S_A_sur=-22.0;
//     double DG0_2S_B_sur=-1.0;
//     double DG0_3S_sur=-22.0;
    //printf("a1a2b1%lf\t a1a2b2%lf\t b1b2a1%lf\t b1b2a2%lf\n",DG0ta1a2b1,DG0ta1a2b2,DG0tb1b2a1,DG0tb1b2a2);
    double Sz=0.0;
    int i,j,q;
    double sum,sum_B;
...
//     double n_alpha = 3.0;
//     double n_T = 5.0;
//     printf("DG0=%lf\t DGT=%lf\t n_alpha=%lf\t n_T=%lf\n",DG0,DGT,n_alpha,n_T);
    for (i=0; i<Np; i++)
    {
        if (P[i].type==0)
        {
...
//         printf("****P[i].nf_a1_old=%lf\n",P[i].nf_a1_old);
//         printf("*****P[i].nf_a2_old=%lf\n",P[i].nf_a2_old);
...
//         printf("*****P[i].nf_b1_old=%lf\n",P[i].nf_b1_old);
//         printf("*****P[i].nf_b2_old=%lf\n",P[i].nf_b2_old);
...
//             printf("test index_cs= %d\n",index_cs);
            double PF = P[index_cs].omega_is/(P[index_cs].omega_i*omega_S*rho0); //eqn4(for suface) arg of log
//             printf("test PF= %lf\n",PF);
//             double DG_ij_surface=DG0_surface_star/beta-log(PF)/beta; //delta G_is
//             printf("DG_ij_surface= %lf\n",DG_ij_surface);
//             betaDG_ij_surface=beta*DG_ij_surface;                   //beta delta G_is
...
//             printf("test PF_L= %lf\n",PF_L);
            ...
            //printf("betaDG_ij= %lf\n",betaDG_ij);
            //printf("123betaDG_ii_L= %lf\n",betaDG_ii_L);
            //sum=sum+P[index_cs].nf_a1_old*exp(-betaDG_ij_surface)+P[index_cs].nf_a1_old*P[index_cs].nf_a2_old*exp(-betaDG_ii_L)*exp(-betaDG_ij_surface);   //eqn S12 3rd eqn summetion part

//             double DG0_S=-12.0;
//             double DG0_L=-10;
//             double DG0_2S_A=-22.0
//             double DG0_2S_B=-1.0
//             double DG0_3S=-22.0

            //printf("DG0_surface_star=%lf",DG0_surface_star);
            //printf("\nexp=%lf\n",PF_L*PF*exp(-DG0_S));
            //printf("\n DG0_S=%lf\n",DG0_S);
...
//                 printf("P[index_cs].nf_a1_old=%lf P[index_cs].nf_a1_old=%lf P[index_cs].nf_a2_old=%lf\n",P[index_cs].nf_a1_old,P[index_cs].nf_a1_old,P[index_cs].nf_a2_old);
//                 printf("PF=%lf PF_L=%lf K0=%lf K1=%lf\n",PF,PF_L);
            }
...
//                 printf("PF_L=%lf\t PF_Lj\n=%lf",PF_L,PF_Lj);
                //double DG_ii_L = DG0_L_star/beta-log(PF_L)/beta;
                //betaDG_ii_L=beta*DG_ii_L;
//                 printf("typei typej=%d %d\n",P[i].type,P[index].type);
//                 printf("a1a2b1%lf\t a1a2b2%lf\t b1b2a1%lf\t b1b2a2%lf\n",DG0ta1a2b1,DG0ta1a2b2,DG0tb1b2a1,DG0tb1b2a2);
                if(P[i].type==0 && P[index].type==1)
                {
//                     printf("inside typei typej=%d %d\n",P[i].type,P[index].type);
//                     printf("DG0_star=%lf\n",DG0_star);
...
//                 double DG_ij_surface=DG0_surface_star/beta-log(PF)/beta;
...
//                 if(P[i].type==0)
//                 {
//                 printf("test sum a1=%lf\n",sum_a1);
//                 printf("test sum a2=%lf\n",sum_a2);
//                 }
//                 if(P[i].type==1)
//                 {
//                 printf("test sum b1=%lf\n",sum_b1);
//                 printf("test sum b2=%lf\n",sum_b2);
//                 }
            }
            double PF_L = 1/(P[i].omega_i*rho0);
//             double DG_ii_L = DG0_L_star/beta-log(PF_L)/beta;
...
//             printf("P[i].nf_a1_old=%lf\t P[i].nf_a1_new=%lf\t error_a1=%lf\n",P[i].nf_a1_old,P[i].nf_a1_new,error_a1);
...
//             printf("error_b2=%lf\n",error_b2);
            ...
            //printf("error_C=%lf\n",error_C);
            //cout << "old=" << P[i].nf_a1_old << " new=" << P[i].nf_a1_new << " error= " << fabs(error) << endl;
            ...
            //if (fabs(error_a1) < 0.0000001 && fabs(error_a2) < 0.0000001 && fabs(error_b1) < 0.0000001 && fabs(error_b2) < 0.0000001) Np_roots=Np_roots+1;
...
//         cout << "iteration= " << Niteration << endl;
    ...
    for (i=0; i<Np; i++)
    {
        ...
        //P[i].Pn[i].nl_d=P[i].nf_a1_old*P[i].nf_a2_old*exp(-betaDG_ii_L); //eqn 3 1st eqn (no.  of loops) (no changes needed)
        ...
        //P[i].Pn[i].nl_d= P[i].Pn[i].nl_d_A+P[i].Pn[i].nl_d_B;
        ...
        {
            int index=P[i].Pn[j].nn_index;
//             printf("\nparticle =%d Ptype=%d neighbor= %d Ntype=%d\n",i,P[i].type,index,P[index].type);

            double PF = P[i].Pn[index].omega_ij/(P[i].omega_i*P[index].omega_i);
            double PF_Lj = 1/(P[index].omega_i*rho0);
            //double PF_L = 1/(P[i].omega_i*rho0);
            //printf("betaDG_ij******= %lf %lf\n",PF,P[i].Pn[index].omega_ij);
            //double DG0_star=beta*DG0-log(1/rho0*CUB(L));
            //double DG_ij=DG0_star/beta-log(PF)/beta;
            //double betaDG_ij=beta*DG_ij;
            //printf("betaDG_ij= %lf %lf\n",betaDG_ij,DG_ij);

            //P[i].Pn[index].nb_d=(P[i].nf_a1_old*P[index].nf_b1_old + P[i].nf_a2_old*P[index].nf_b2_old )*exp(-betaDG_ij);
...
//             printf("P[i].Pn[index].nb_d=%lf\n",P[i].Pn[index].nb_d);
//             printf("PF=%lf\n",PF);
//             printf("PF_L=%lf\n",PF_L);
//             printf("DG0t=%lf\n",DG0t);
//
//             double expo=exp(-DG0t)*PF*PF_L;
//             printf("expo=%lf",expo);

            if(P[i].type==0 && P[index].type==1)
            {
            //P[i].Pn[index].nb_d3S_a1=P[i].nf_a1_old*P[i].nf_a2_old*P[index].nf_b1_old*exp(-betaDG_ij)*exp(-betaDG_ii_L)+P[i].nf_a1_old*P[i].nf_a2_old*P[index].nf_b2_old*exp(-betaDG_ij)*exp(-betaDG_ii_L)+P[index].nf_b1_old*P[index].nf_b2_old*P[i].nf_a1_old*exp(-betaDG_ij)*exp(-betaDG_ii_L);

            //P[i].Pn[index].nb_d3S_a2=P[i].nf_a1_old*P[i].nf_a2_old*P[index].nf_b1_old*exp(-betaDG_ij)*exp(-betaDG_ii_L)+P[i].nf_a1_old*P[i].nf_a2_old*P[index].nf_b2_old*exp(-betaDG_ij)*exp(-betaDG_ii_L)+P[index].nf_b1_old*P[index].nf_b2_old*P[i].nf_a2_old*exp(-betaDG_ij)*exp(-betaDG_ii_L);
...
//             printf("expo=%lf",expo);
            if(P[i].type==1 && P[index].type==0)
            {
            //P[i].Pn[index].nb_d3S_b1=P[index].nf_a1_old*P[index].nf_a2_old*P[i].nf_b1_old*exp(-betaDG_ij)*exp(-betaDG_ii_L)+P[i].nf_b1_old*P[i].nf_b2_old*P[index].nf_a1_old*exp(-betaDG_ij)*exp(-betaDG_ii_L)+P[i].nf_b1_old*P[i].nf_b2_old*P[index].nf_a2_old*exp(-betaDG_ij)*exp(-betaDG_ii_L);

            //P[i].Pn[index].nb_d3S_b2=P[index].nf_a1_old*P[index].nf_a2_old*P[i].nf_b2_old*exp(-betaDG_ij)*exp(-betaDG_ii_L)+P[i].nf_b1_old*P[i].nf_b2_old*P[index].nf_a1_old*exp(-betaDG_ij)*exp(-betaDG_ii_L)+P[i].nf_b1_old*P[i].nf_b2_old*P[index].nf_a2_old*exp(-betaDG_ij)*exp(-betaDG_ii_L);
...
//                 printf("P[i].Pn[index].nb_d_a1**=%lf\n",P[i].Pn[index].nb_d_a1);
//                 printf("P[i].Pn[index].nb_d_a2**=%lf\n",P[i].Pn[index].nb_d_a2);
//                 printf("tot_a1_in_Bridge1=%lf\n",tot_a1_in_Bridge);
//                 printf("tot_a2_in_Bridge1=%lf\n",tot_a2_in_Bridge);
            }
...
//                 printf("P[i].Pn[index].nb_d_a1_AA=%lf\n",P[i].Pn[index].nb_d_a1_AA);
//                 printf("P[i].Pn[index].nb_d_a2_AA=%lf\n",P[i].Pn[index].nb_d_a2_AA);
//                 printf("tot_a1_in_Bridge2=%lf\n",tot_a1_in_Bridge);
//                 printf("tot_a2_in_Bridge2=%lf\n",tot_a2_in_Bridge);
            }
...
//                 printf("P[i].Pn[index].nb_d_b1**=%lf\n",P[i].Pn[index].nb_d_b1);
//                 printf("P[i].Pn[index].nb_d_b2**=%lf\n",P[i].Pn[index].nb_d_b2);
            }
...
//                 printf("P[i].Pn[index].nb_d_b1_BB=%lf\n",P[i].Pn[index].nb_d_b1_BB);
//                 printf("P[i].Pn[index].nb_d_b2_BB=%lf\n",P[i].Pn[index].nb_d_b2_BB);
            }
...
//             if(P[i].type==0 && P[index].type==1)
//             {
//             printf("\ntot_a1_in_3SBridge=%lf\n",tot_a1_in_3SBridge);
//             printf("\ntot_a2_in_3SBridge=%lf\n",tot_a2_in_3SBridge);
//             }
//             if(P[i].type==1 && P[index].type==0)
//             {
//             printf("\ntot_b1_in_3SBridge=%lf\n",tot_b1_in_3SBridge);
//             printf("\ntot_b2_in_3SBridge=%lf\n",tot_b2_in_3SBridge);
//             }
...
//                 printf("P[i].nf_a1_old=%lf\t P[i].nf_a2_old=%lf\t P[index].nf_b1_old=%lf\t P[index].nf_b2_old=%lf\t PF=%lf\t PF_L=%lf\n",P[i].nf_a1_old,P[i].nf_a2_old,P[index].nf_b1_old,P[index].nf_b2_old,PF,PF_L);
//                 printf("P[i].Pn[index].nb_d3S_A=%lf\n",P[i].Pn[index].nb_d3S_A);

//                 P[i].Pn[index].nb_d3S=P[i].Pn[index].nb_d3S_A;
                //P[i].Pn[index].nb_d3S = P[i].nf_a1_old*P[i].nf_a2_old*P[index].nf_b1_old*exp(-DG0t)*PF*PF_L+P[i].nf_a1_old*P[i].nf_a2_old*P[index].nf_b2_old*exp(-DG0t)*PF*PF_L+P[index].nf_b1_old*P[index].nf_b2_old*P[i].nf_a1_old*exp(-DG0t)*PF*PF_L+P[index].nf_b1_old*P[index].nf_b2_old*P[i].nf_a2_old*exp(-DG0t)*PF*PF_L;
            }
            if(P[i].type==1 && P[index].type==0)
            {
...
//                 printf("P[index].nf_a1_old=%lf\t P[index].nf_a2_old=%lf\t P[i].nf_b1_old=%lf\t P[i].nf_b2_old=%lf\t PF=%lf\t PF_L=%lf\n",P[index].nf_a1_old,P[index].nf_a2_old,P[i].nf_b1_old,P[i].nf_b2_old,PF,PF_L);
//                 printf("P[i].Pn[index].nb_d3S_B=%lf\n",P[i].Pn[index].nb_d3S_B);

//                 P[i].Pn[index].nb_d3S=P[i].Pn[index].nb_d3S_B;
                //P[i].Pn[index].nb_d3S = P[index].nf_a1_old*P[index].nf_a2_old*P[i].nf_b1_old*exp(-DG0t)*PF*PF_L+P[i].nf_b1_old*P[i].nf_b2_old*P[index].nf_a1_old*exp(-DG0t)*PF*PF_L+P[i].nf_b1_old*P[i].nf_b2_old*P[index].nf_a2_old*exp(-DG0t)*PF*PF_L+P[index].nf_a1_old*P[index].nf_a2_old*P[i].nf_b2_old*exp(-DG0t)*PF*PF_L;
            }
...
//             printf("total_3S_bridge=%lf",total_3S_bridge);

            if(P[i].type==1)
            {
                total_3S_bridge=total_3S_bridge+P[i].Pn[index].nb_d3S_B;
            }
//             printf("total_3S_bridge=%lf\n",total_3S_bridge);

            //P[i].Pn[index].nb_d3S=P[i].Pn[index].nb_d3S_A+P[i].Pn[index].nb_d3S_B;
            //printf("Ptype=%d\t Ntype=%d\t P[i].Pn[index].nb_d3S_A=%lf\t P[i].Pn[index].nb_d3S_B=%lf\n",P[i].type,P[index].type,P[i].Pn[index].nb_d3S_A,P[i].Pn[index].nb_d3S_B);
//             printf("Ptype=%d\t Ntype=%d\t P[i].Pn[index].nb_d3S_A=%lf\n",P[i].type,P[index].type,P[i].Pn[index].nb_d3S_A);

            //total_bridge=total_bridge+P[i].Pn[index].nb_d;
            //printf("\ntotal bridge=%lf\n\n",total_bridge);
        }
        //P[i].total_bridge_WC=total_bridge;
        //P[i].total_free_loops=P[i].nf_a1_old+P[i].nf_a2_old+P[i].Pn[i].nl_d;
        if(P[i].type==0)
        {
            //P[i].total_free_loops=P[i].nf_a1_old+P[i].nf_a2_old+P[i].Pn[i].nl_d;
        ...
        if(P[i].type==1)
        {
            //P[i].total_free_loops=P[i].nf_b1_old+P[i].nf_b2_old+P[i].Pn[i].nl_d;
            P[i].total_free_loops_B=P[i].nf_b1_old+P[i].nf_b2_old+P[i].Pn[i].nl_d;
        }
//         printf("P[i].total_free_loops_A=%lf\n",P[i].total_free_loops_A);
//         printf("P[i].total_free_loops_B=%lf\n",P[i].total_free_loops_B);

        if(P[i].type==0)
        {
        total_bridge=tot_a1_in_Bridge+tot_a2_in_Bridge;
        }
//         printf("tot_a1_in_Bridge=%lf\n",tot_a1_in_Bridge);
//         printf("tot_a2_in_Bridge=%lf\n",tot_a2_in_Bridge);
//         printf("total_bridge=%lf\n",total_bridge);
        if(P[i].type==1)
        {
        total_bridge=tot_b1_in_Bridge+tot_b2_in_Bridge;
        }
//         printf("tot_b1_in_Bridge=%lf\n",tot_b1_in_Bridge);
//         printf("tot_b2_in_Bridge=%lf\n",tot_b2_in_Bridge);
//         printf("total_bridge=%lf\n",total_bridge);

//         P[i].total_3S_bridge=total_3S_bridge;
//         printf("P[i].total_3S_bridge=%lf",P[i].total_3S_bridge);
    }
...
//         betaDG_ij_surface=beta*DG_ij_surface;
        ...
        //P[index_cs].nbS_d=P[index_cs].nf_a1_old * *nfS_C_old * exp(-betaDG_ij_surface)+P[index_cs].nf_b2_old * *nfS_C_old * exp(-betaDG_ij_surface); //eqn 3,4th eqn, no changes needed

//         printf("no. of free a1=%lf\n",P[index_cs].nf_a1_old);
//         printf("no. of free a2=%lf\n",P[index_cs].nf_a2_old);
//         printf("no. of free b1=%lf\n",P[index_cs].nf_b1_old);
//         printf("no. of free b2=%lf\n",P[index_cs].nf_b2_old);
//         printf("2nfS_C_old= %lf\n",*nfS_C_old);
//         printf("betaDG_ij_surface= %lf\n",betaDG_ij_surface);
//         printf("exp(-betaDG_ij_surface)= %lf\n",exp(-betaDG_ij_surface));

        if(P[index_cs].type==0)
        {
            //P[index_cs].nbS_d=P[index_cs].nf_a1_old * *nfS_C_old * exp(-betaDG_ij_surface);
            P[index_cs].nbS_d_A=P[index_cs].nf_a1_old * *nfS_C_old *PF*K0;
        }
        if(P[index_cs].type==1)
        {
            //P[index_cs].nbS_d=P[index_cs].nf_b2_old * *nfS_C_old * exp(-betaDG_ij_surface);
            P[index_cs].nbS_d_B=P[index_cs].nf_b2_old * *nfS_C_old *PF*K0;
        }

        P[index_cs].nbS_d=P[index_cs].nbS_d_A+P[index_cs].nbS_d_B;
        //printf("type=%d P[index_cs].nbS_d_A=%lf\n",P[index_cs].type,P[index_cs].nbS_d_A);
        //printf("type=%d P[index_cs].nbS_d_B=%lf\n",P[index_cs].type,P[index_cs].nbS_d_B);
//         printf("Ptype=%d\t [index_cs].nbS_d=%lf\n",P[i].type,P[index_cs].nbS_d);
//         printf("2nfS_C_old= %lf\n",*nfS_C_old);
...
//             term1x = (P[i].Pn[index].nb_d) * ( (P[i].Pn[index].omega_ij_prime*ux) /P[i].Pn[index].omega_ij);
//             term1y = (P[i].Pn[index].nb_d) * ( (P[i].Pn[index].omega_ij_prime*uy) /P[i].Pn[index].omega_ij);
//             term1z = (P[i].Pn[index].nb_d) * ( (P[i].Pn[index].omega_ij_prime*uz) /P[i].Pn[index].omega_ij);
...
//                  term1x = (P[i].Pn[index].nb_d+P[i].Pn[index].nb_d3S) * ( (P[i].Pn[index].omega_ij_prime*ux) /P[i].Pn[index].omega_ij);
//                  term1y = (P[i].Pn[index].nb_d+P[i].Pn[index].nb_d3S) * ( (P[i].Pn[index].omega_ij_prime*uy) /P[i].Pn[index].omega_ij);
//                  term1z = (P[i].Pn[index].nb_d+P[i].Pn[index].nb_d3S) * ( (P[i].Pn[index].omega_ij_prime*uz) /P[i].Pn[index].omega_ij);
...
//             sum_term1z=sum_term1z+term1z;
            ...
            if(P[i].type==0 && P[index].type==1)
            {
//              sum_term1x_01=sum_term1x_01+term1x_01;
//              sum_term1y_01=sum_term1y_01+term1y_01;
//              sum_term1z_01=sum_term1z_01+term1z_01;
            ...
            if(P[i].type==1 && P[index].type==0)
            {
//              sum_term1x_10=sum_term1x_10+term1x_10;
//              sum_term1y_10=sum_term1y_10+term1y_10;
//              sum_term1z_10=sum_term1z_10+term1z_10;
            ...
            if(P[i].type==0 && P[index].type==0)
            {
//              sum_term1x_00=sum_term1x_00+term1x_00;
//              sum_term1y_00=sum_term1y_00+term1y_00;
//              sum_term1z_00=sum_term1z_00+term1z_00;
            ...
            if(P[i].type==1 && P[index].type==1)
            {
//              sum_term1x_11=sum_term1x_11+term1x_11;
//              sum_term1y_11=sum_term1y_11+term1y_11;
//              sum_term1z_11=sum_term1z_11+term1z_11;
...
//             sum_term2z = sum_term2z + term2z;
            ...
            if(P[i].type==0 && P[index].type==1)
            {
//             sum_term2x_01 = sum_term2x + term2x_01;
//             sum_term2y_01 = sum_term2y + term2y_01;
//             sum_term2z_01 = sum_term2z + term2z_01;
            ...
            if(P[i].type==1 && P[index].type==0)
            {
//             sum_term2x_10 = sum_term2x_10 + term2x_10;
//             sum_term2y_10 = sum_term2y_10 + term2y_10;
//             sum_term2z_10 = sum_term2z_10 + term2z_10;
            ...
            if(P[i].type==0 && P[index].type==0)
            {
//             sum_term2x_00 = sum_term2x_00 + term2x_00;
//             sum_term2y_00 = sum_term2y_00 + term2y_00;
//             sum_term2z_00 = sum_term2z_00 + term2z_00;
            ...
            if(P[i].type==1 && P[index].type==1)
            {
//             sum_term2x_11 = sum_term2x_11 + term2x_11;
//             sum_term2y_11 = sum_term2y_11 + term2y_11;
//             sum_term2z_11 = sum_term2z_11 + term2z_11;
...
//     if(P[i].type==0 && P[index].type==1)
//     {
//         P[i].Fx_01=(sum_term1x_01-sum_term2x_01-sum_term3x)/beta;
//         P[i].Fy_01=(sum_term1y_01-sum_term2y_01-sum_term3y)/beta;
//         P[i].Fz_01=(sum_term1z_01-sum_term2z_01-sum_term3z)/beta;
//     }
//     if(P[i].type==1 && P[index].type==0)
//     {
//         P[i].Fx_10=(sum_term1x_10-sum_term2x_10-sum_term3x)/beta;
//         P[i].Fy_10=(sum_term1y_10-sum_term2y_10-sum_term3y)/beta;
//         P[i].Fz_10=(sum_term1z_10-sum_term2z_10-sum_term3z)/beta;
//     }
//     if(P[i].type==0 && P[index].type==0)
//     {
//         P[i].Fx_00=(sum_term1x_00-sum_term2x_00-sum_term3x)/beta;
//         P[i].Fy_00=(sum_term1y_00-sum_term2y_00-sum_term3y)/beta;
//         P[i].Fz_00=(sum_term1z_00-sum_term2z_00-sum_term3z)/beta;
//     }
//     if(P[i].type==1 && P[index].type==1)
//     {
//         P[i].Fx_11=(sum_term1x_11-sum_term2x_11-sum_term3x)/beta;
//         P[i].Fy_11=(sum_term1y_11-sum_term2y_11-sum_term3y)/beta;
//         P[i].Fz_11=(sum_term1z_11-sum_term2z_11-sum_term3z)/beta;
//     }
//     //}
//     if(P[i].type==0)
//     {
//         P[i].Fx=P[i].Fx_01+P[i].Fx_00;
//         P[i].Fy=P[i].Fy;
//         P[i].Fz=P[i].Fz;
//     }
    ...
    for (q=0; q<ns; q++)
    {
        ...
        //term1z=  P[index_cs].nbS_d * ( (P[index_cs].omega_is_prime*uz) / P[index_cs].omega_is );
        ...
        //term1z=  (P[index_cs].nbS_d+P[index_cs].lfb) * ( (P[index_cs].omega_is_prime*uz) / P[index_cs].omega_is );

//         term2z=(P[index_cs].total_free_loops) *((P[index_cs].e_is_prime*uz)/P[index_cs].omega_i)+
//             nfS_C_old*((P[index_cs].e_si_prime*uz)/omega_S);
...
double Isolated_colloids(int Na1, int Na2, double DG0_L_star,double beta,double rho0, double omega_0)
{
    ...
    //double betaFreeEnergy_bulk=Na1*log((nf_A_bulk*nf_B_bulk)/(Na1*Na2))+(Na1-nf_A_bulk);
