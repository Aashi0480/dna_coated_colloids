...
# Function to read the first dataset and extract values
...
# Function to read the second dataset and extract values
def plot_second_data(file_path):
    df = pd.read_csv(file_path, sep="\t")
    df = df.dropna(subset=['avg_z', 'err_z', 'avg_na1_na2_nb1_nb2', 'err_na1_na2_nb1_nb2'])
...
# Function to read theory points safely
def read_theory_data(file_path):
    import numpy as np
    with open(file_path, 'r') as file:
        code = file.read()
    local_vars = {}
    exec(code, {'np': np}, local_vars)
    return local_vars['n_loop_3']
...
def read_and_plot_data(file_path1, file_path2, theory_file):
    ...
    plt.savefig('combined_plot_with_theory.png',
                dpi=300,
                bbox_inches='tight')
